...
export function renderBalance(
  title: string,
  accounts: BalanceStock[]
): HTMLElement {
  const container = document.createElement("div");
  const active: BalanceStock[] = [];
  const passive: BalanceStock[] = [];

  accounts.forEach((x) => {
    const element: BalanceStock = {
      item: { ...x.item, name: x.item.name },
      value: x.value,
    };
...
function createBalance(
  ...
  while (passiveIndex < count || activeIndex < count) {
    ...
    if (activeIndex < active.length) {
      if (lastActiveCategory !== active[activeIndex].item.category) {
        ...
        console.log(`aaa active: ${activeIndex}, passive: ${passiveIndex}`);
        if (category) {
          console.log(`bbb active: ${activeIndex}, passive: ${passiveIndex}`);
          row.appendChild(createCategory(category));
          activeCategories++;
        } else {
          console.log(`ccc active: ${activeIndex}, passive: ${passiveIndex}`);
          populateEntrySide(row, active[activeIndex++], true);
        }
      } else {
        console.log(`ddd active: ${activeIndex}, passive: ${passiveIndex}`);
        populateEntrySide(row, active[activeIndex++], true);
      }
    } else if (activeIndex == active.length) {
      console.log(`eee active: ${activeIndex}, passive: ${passiveIndex}`);
      populateEmptyRow(row, count - activeIndex, true);
      activeIndex = count;
    }

    console.log(`fff active: ${activeIndex}, passive: ${passiveIndex}`);
    if (passiveIndex < passive.length) {
      console.log(`ggg active: ${activeIndex}, passive: ${passiveIndex}`);
      populateEntrySide(row, passive[passiveIndex], false);
    } else if (passiveIndex == passive.length) {
      console.log(`hhh active: ${activeIndex}, passive: ${passiveIndex}`);
      populateEmptyRow(row, count - passiveIndex + activeCategories, false);
      passiveIndex = count;
    }

    console.log(`iiii active: ${activeIndex}, passive: ${passiveIndex}`);
    passiveIndex++;

    table.appendChild(row);
  }

  console.log(`ee active: ${activeIndex}, passive: ${passiveIndex}`);
  ...
  console.log(`yy active: ${activeIndex}, passive: ${passiveIndex}`);
  container.className = "container w-full d-block";
  container.appendChild(createHeader(accountName));
  container.appendChild(table);

  console.log(`y<< active: ${activeIndex}, passive: ${passiveIndex}`);
  verifyAccount(
    container,
    activeSum,
    passiveSum,
